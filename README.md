...
```bash
GET /api-docs            # Інтерактивна документація (Swagger)
GET /api/health          # Стан сервісу та Telegram
GET /api/schedule/all    # Всі доступні графіки
GET /api/info            # Мета-дані та список усіх API
```
...
| Ендпоінт | Метод | Опис |
|----------|-------|------|
| `/api-docs` | GET | **Swagger UI** (рекомендовано для тестування) |
| `/api/health` | GET | Стан сервісу, uptime та статус Telegram |
| `/api/schedule/current` | GET | Поточний активний графік |
| `/api/schedule/future` | GET | Майбутній запланований графік |
| `/api/schedule/all` | GET | Список усіх збережених графіків |
| `/api/schedule/history` | GET | Історія повідомлень (параметр `?limit=10`) |
| `/api/refresh` | POST | Примусовий запуск парсингу каналу |
| `/api/debug/dates` | GET | Перевірка системного часу сервера |
| `/api/info` | GET | Мета-дані та перелік усіх точок доступу |
| `/api/setup/*` | ANY | Ендпоінти для веб-налаштування (див. [WEB_SETUP.md](docs/WEB_SETUP.md)) |

Докладна інтерактивна документація доступна за адресою: `http://localhost:8080/docs`
