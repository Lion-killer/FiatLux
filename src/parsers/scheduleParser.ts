...
export class ScheduleParser {
  ...
  static parseMessage(message: Api.Message): Schedule | null {
    ...
    try {
      ...
      if (!date) {
        logger.warn('Could not extract date from message');
        return null;
      }
      ...
      if (queues.length === 0) {
        logger.debug('Skipping message - no queues found');
        return null;
      }
      ...
      if (!hasTimeSlots) {
        logger.debug('Skipping message - no time slots found in queues');
        return null;
      }
